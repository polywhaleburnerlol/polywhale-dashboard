...
export async function registerClient(formData: FormData): Promise<ActionResult> {
  ...
  const requiredFields: (keyof RegisterClientInput)[] = [
    "label", "funder_address", "private_key", "poly_api_key", "poly_secret", "poly_passphrase",
  ];
  for (const field of requiredFields) {
    if (!raw[field]) return { success: false, error: `Missing required field: ${field}` };
  }
...
export async function addWhaleAddress(address: string, label: string): Promise<AddWhaleResult> {
  const addr = (address ?? "").trim().toLowerCase();
  if (!addr.startsWith("0x") || addr.length !== 42) {
    return { success: false, error: "Must be a valid 42-character Ethereum address (0x…)." };
  }
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase
    .from("whale_watchlist")
    .upsert({ address: addr, label: label || null }, { onConflict: "address" })
    .select("id")
    .single();
  if (error) return { success: false, error: `Database error: ${error.message}` };
...
export async function removeWhaleAddress(id: string): Promise<RemoveResult> {
...
export async function toggleClientActive(clientId: string, isActive: boolean): Promise<ToggleResult> {
  if (!clientId) return { success: false, error: "Missing client ID." };
  const supabase = getSupabaseAdmin();
